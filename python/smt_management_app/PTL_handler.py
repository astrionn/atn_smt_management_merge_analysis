...
class PTL_API:
    ...
    def LED_slot_control(self,gateway=1,controller=1,command=11,channel=1,LED=1,R=0,G=0,B=0):

        if LED > len(self.slot_to_strip_map):
            raise Exception(f"Value {LED} for slot ID is out of range {len(self.slot_to_strip_map)}")
    ...
    def LED_slot_code_control(self,gateway=1,controller=1,command=11,code=1001,R=0,G=0,B=0):
